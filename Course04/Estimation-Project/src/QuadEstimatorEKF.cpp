...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F attitude_iner = attitude.Rotate_BtoI(accel);

  predictedState(0) += curState(3) * dt;
  predictedState(1) += curState(4) * dt;
  predictedState(2) += curState(5) * dt;
  predictedState(3) += attitude_iner.x * dt;
  predictedState(4) += attitude_iner.y * dt;
  predictedState(5) += attitude_iner.z * dt - CONST_GRAVITY * dt;
  // yaw integral is already done in the IMU update
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0, 0) = -cos_theta * cos_psi;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  VectorXf RbgPrime_ut_dt = RbgPrime * accelVec * dt;

  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = RbgPrime_ut_dt(0);
  gPrime(4, 6) = RbgPrime_ut_dt(1);
  gPrime(5, 6) = RbgPrime_ut_dt(2);
